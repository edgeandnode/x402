...
import { DeferredPaymentRequirements, evm, PaymentRequirements } from "x402/types";
import { wrapFetchWithPayment, wrapFetchWithDeferredPayment } from "./index";
...
describe("fetchWithDeferredPayment()", () => {
  let mockFetch: ReturnType<typeof vi.fn>;
  let mockWalletClient: typeof evm.SignerWallet;
  let wrappedDeferredFetch: ReturnType<typeof wrapFetchWithDeferredPayment>;
  ...
  beforeEach(async () => {
    ...
    wrappedDeferredFetch = wrapFetchWithDeferredPayment(mockFetch, mockWalletClient);
  });
  ...
  it("should return the original response for non-402 status codes", async () => {
    ...
    const result = await wrappedDeferredFetch("https://api.example.com");
  ...
  it("should not retry if already retried", async () => {
    ...
    await expect(
      wrappedDeferredFetch("https://api.example.com", {
        __is402Retry: true,
      } as RequestInitWithRetry),
    ).rejects.toThrow("Payment already attempted");
  });
  ...
  it("should reject if missing request config", async () => {
    ...
    await expect(wrappedDeferredFetch("https://api.example.com")).rejects.toThrow(
      "Missing fetch request configuration",
    );
  });

  it("should reject if payment amount exceeds maximum transaction amount", async () => {
    const errorResponse = createResponse(402, {
      accepts: [
        {
          ...validPaymentRequirements[0],
    ...
    ).rejects.toThrow("Payment amount exceeds maximum request allowed amount");
  });
